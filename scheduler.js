...
const processes_html = new Object()

const qtd_nucleos = 8
const processos_gerar = 20
const timeout = 120
...
const processos = new Object()
const tickets_processos = new Array()
let tickets_total = 0
...
function gerar_processos(qtd_processos) {
    for (let id = 1; id <= qtd_processos; id++) {
        const duracao_processo = Math.floor(Math.random() * 3600) + 10;
...
const processos_nucleo = new Object()
const tickets_nucleo = new Object()
...
async function lottery(nucleo) {
    while (removeEmpty(processos_nucleo[nucleo]).length > 0) {
        ...
        let processo_sorteado = nucleo_tickets[Math.floor(Math.random() * nucleo_tickets.length)]

        nucleo_processos.forEach(processo => {
            if (processo.id == processo_sorteado) {
                processo_sorteado = processo
            }
        })
        ...
        try { await processo_sorteado.executar(tempo_processamento, nucleo) }
        catch (err) {
            continue
        }

        document.getElementById(`core${nucleo}`).innerHTML = `Núcleo ${nucleo}: ID ${String(processo_sorteado.id || 0).padStart(3, '0')}`
